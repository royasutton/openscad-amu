...
################################################################################
#
#  Copyright (C) 2016-2023,2026 Roy Allen Sutton
#
...
$(this_guard) := $(notdir $(this_file))
  ...
  ifeq ($(gnumake_vmin),$(firstword $(sort $(MAKE_VERSION) $(gnumake_vmin))))
	...
	#------------------------------------------------------------------------------#
	# AMU_PM_GET_COMPONENTS (arg1,arg2,arg3,arg4)
	# arg1: components path prefix.
	# arg2: components file suffix.
	# arg3: list of component types.
	# arg4: list of component sub-section/groups.
	# returns list of existing makefile components.
	#------------------------------------------------------------------------------#
  define AMU_PM_GET_COMPONENTS
    $(amu_pm_d4)$(foreach p,$3,$(foreach s,$4,
      $(if $(wildcard $(strip $1)$p_$s$(strip $2)),
        $(strip $1)$p_$s$(strip $2),
        $(call amu_pm_dm,skipping [$(strip $1)$p_$s$(strip $2)], does not exists.)
      )
    ))
  endef
  ...
  ifndef AMU_PM_PREFIX
	...
	# Default include file suffix (update iff not defined; use ?=)
  ifndef AMU_PM_SUFFIX
    AMU_PM_SUFFIX	?= $(suffix $(this_file))
    $(call amu_pm_m,setting: AMU_PM_SUFFIX=$(AMU_PM_SUFFIX))
  endif
  ...
  ifdef AMU_PM_COMPONENTS_ADD
	...
	# Defaults for local components
	# AMU_PM_PREFIX_LOCAL
	# AMU_PM_COMPONENTS_LOCAL

  ifndef AMU_PM_SUFFIX_LOCAL
    AMU_PM_SUFFIX_LOCAL	?= $(AMU_PM_SUFFIX)
    $(call amu_pm_m,setting: AMU_PM_SUFFIX_LOCAL=$(AMU_PM_SUFFIX_LOCAL))
  endif
