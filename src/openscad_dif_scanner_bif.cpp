...
/***************************************************************************//**
  ...
  \details
    ...
    argument one-by-one (skipping function name):

    \em Named argument value pairs are stored to the local variable
    scope by default (ie: name=value). A \em flag arguments will store
    a value of 1 or -1 (ie: ++i will store i=1). The special flags \em
    ++/--global and \em ++/--local control if the respective variable
    scope will be updated for subsequent assignments.
    ...
    \em Incr/decr will always operate on and update the global variable
    scope (ex: i++ or i-- will always increment or decrement the global
    variable i).

    Positional arguments are expanded and appended to the return
    result. Each argument is separated by a configurable feild
    separator (default: ${EFS}='[:space:]'). Escaped variables are
    reduced and checked during subsequent expansions. Delayed expansion
    is required to access the value of a variable stored using the \em
    flag syntax.
...
/***************************************************************************//**
  ...
  \details
    ...
    The flag \p pstarget is used to distinguish between the target origins.
...
/***************************************************************************//**
  \details
    ...
    Prefix flags with \c ++ to enable and \c \-\- to disable. For
    example <tt>++verbose</tt> will set the flag value to true. The
    options and flags (and their short codes) are summarized in the
    following tables.
